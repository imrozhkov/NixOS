...
'';
  ...
  # Создаём ~/.ssh и чиним права с корректной группой текущего пользователя
  ...
  services.hypridle = {
    ...
    settings = {
      general = {
        lock_cmd = "pidof gtklock >/dev/null || gtklock";
        before_sleep_cmd = "loginctl lock-session";
        after_sleep_cmd = "hyprctl dispatch dpms on";
      };
      listener = [
        { timeout = 300; on-timeout = "pidof gtklock >/dev/null || gtklock"; }
        {
          timeout = 305;
